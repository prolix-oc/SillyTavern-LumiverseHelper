...
if [ "$SKIP_UPDATE" = false ]; then
    ...
    # Also update the version.js file to avoid manifest caching issues
    VERSION_JS_FILE="src/lib/version.js"
    if [ -f "$VERSION_JS_FILE" ]; then
        echo "Updating version.js constant..."
        sed -i.bak "s/export const EXTENSION_VERSION = \"[^\"]*\";/export const EXTENSION_VERSION = \"$NEW_VERSION\";/" "$VERSION_JS_FILE"
        rm "$VERSION_JS_FILE.bak"
    else
        echo "Warning: $VERSION_JS_FILE not found. Creating it..."
        mkdir -p "src/lib"
...
EOF
    ...
    # Stage the version.js file for commit
    git add "$VERSION_JS_FILE"
fi
