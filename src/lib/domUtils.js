...
const DOMUtils = {
  ...
  /**
   * Get the height of the top bar element
   * Defaults to 50px if not found
   * 
   * @returns {number} Top bar height in pixels
   */
  getTopBarHeight() {
    const topBar = document.getElementById('top-bar');
    return topBar ? topBar.offsetHeight : 50;
  },

  /**
   * Get a safe z-index for the landing page
   * Attempts to find the top bar's z-index and return one less
   * Defaults to 2999 if not found (assuming top bar is ~3000)
   * 
   * @returns {number} Z-index value
   */
  getSafeLandingPageZIndex() {
    const topBar = document.getElementById('top-bar');
    if (topBar) {
      const z = window.getComputedStyle(topBar).zIndex;
      if (z && z !== 'auto') {
        const val = parseInt(z, 10);
        return isNaN(val) ? 2999 : val - 1;
      }
    }
    return 2999;
  }
