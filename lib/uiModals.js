 ...
 * Positioned below top-settings-holder and before sheld
 */
export function createLoomSummaryButton() {
  // Remove existing button if any
  $("#lumia-loom-summary-btn").remove();

  const buttonHtml = `
    <button id="lumia-loom-summary-btn" class="lumia-loom-summary-btn" title="View/Edit Loom Summary">
      <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2">
  ...
  // Insert before #sheld (the main chat area holder)
  const sheld = $("#sheld");
  if (sheld.length) {
    sheld.before(buttonHtml);
  } else {
...
export function updateLoomSummaryButtonState() {
  const $btn = $("#lumia-loom-summary-btn");
  ...
  if (!hasChat) {
    $btn.addClass("lumia-loom-summary-btn-hidden");
    $btn.attr("title", "No active chat");
  } else if (hasSummary) {
    $btn.removeClass("lumia-loom-summary-btn-hidden lumia-loom-summary-btn-empty");
    $btn.addClass("lumia-loom-summary-btn-active");
    $btn.attr("title", "View/Edit Loom Summary");
  } else {
    $btn.removeClass("lumia-loom-summary-btn-hidden lumia-loom-summary-btn-active");
    $btn.addClass("lumia-loom-summary-btn-empty");
    $btn.attr("title", "No Loom Summary (click to create)");
  }
}
