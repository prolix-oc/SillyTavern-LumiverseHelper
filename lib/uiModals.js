...
export function showSelectionModal(type) {
    ...
    if (packs.length === 0) {
        ...
        packs.forEach(pack => {
            ...
            // Render Items - use card layout for definition/behavior/personality
                ...
                if (useCardLayout) {
                    ...
                    <div class="lumia-card ${cardClass} ${isSelected ? 'selected' : ''}"
                        ...
                        <div class="lumia-card-info">
                            ...
                            <div class="lumia-card-pack">${pack.name}</div>
                        </div>
                    </div>
                    `;
                } else {
            ...
            const gridClass = useCardLayout ? 'lumia-card-grid' : 'lumia-grid';

            contentHtml += `
            <div class="lumia-pack-section">
                <div class="lumia-pack-header">
                    <h4>${pack.name} (${packItems.length})</h4>
                    <button class="lumia-icon-btn lumia-remove-pack-btn" data-pack="${escapeHtml(pack.name)}" title="Remove Pack">
                        ${SVG_ICONS.trash}
                    </button>
                </div>
                <div class="${gridClass}">
    ...
    // Add subtitle hint for behavior/personality modals
    let subtitleHtml = '';
    if (dominantKey) {
        subtitleHtml = '<p style="margin: 0 0 10px 0; font-size: 13px; color: rgba(255,255,255,0.6); text-align: center;">Tap the star to mark a dominant trait</p>';
    }
    ...
    const modalHtml = `
        <dialog id="lumia-selection-modal" class="popup wide_dialogue_popup large_dialogue_popup vertical_scrolling_dialogue_popup popup--animation-fast">
            <div class="popup-header">
                <div class="lumia-modal-title-row">
    ...
    const cardSelector = useCardLayout ? ".lumia-card" : ".lumia-grid-item";
    $modal.find(cardSelector).click(function(e) {
...
export function showMiscFeaturesModal() {
    ...
    const modalHtml = `
        <dialog id="lumia-misc-modal" class="popup wide_dialogue_popup large_dialogue_popup popup--animation-fast lumia-modal">
...
export function showSummarizationModal() {
    ...
    const modalHtml = `
        <dialog id="lumia-summarization-modal" class="popup wide_dialogue_popup large_dialogue_popup vertical_scrolling_dialogue_popup popup--animation-fast lumia-modal">
            <div class="lumia-modal-header">
                <div class="lumia-modal-header-icon">
                    <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round">
