...
export function showLoomSelectionModal(category) {
    ...
    $modal.find(".lumia-remove-pack-btn").click(function(e) {
        ...
        if (confirm(`Are you sure you want to remove the pack "${packName}"?`)) {
            ...
            settings.selectedLoomStyle = settings.selectedLoomStyle.filter(s => s.packName !== packName);
            settings.selectedLoomUtils = settings.selectedLoomUtils.filter(s => s.packName !== packName);
            settings.selectedLoomRetrofits = settings.selectedLoomRetrofits.filter(s => s.packName !== packName);
...
export function refreshUIDisplay() {
    ...
    if (packs.length > 0) {
        ...
        if (currentLoomStyleDiv) {
            const names = settings.selectedLoomStyle.map(sel => {
                const item = getItemFromLibrary(sel.packName, sel.itemName);
                return item ? item.loomName : null;
            }).filter(n => n);

            currentLoomStyleDiv.textContent = names.length > 0 ? names.join(", ") : "No styles selected";
        }
